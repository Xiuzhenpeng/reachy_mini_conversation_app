...
from reachy_mini_conversation_app.myself_openai_text import MyselfOpenAITextGenerator
...
class BaseRealtimeHandler(ConversationHandler, ABC):
    ...
    def __init__(
        ...
        self._myself_text_generator: MyselfOpenAITextGenerator | None = None
        self._myself_text_tasks: set[asyncio.Task[None]] = set()
        self._suppress_next_realtime_transcript = False
        if (config.MYSELF_OPENAI_API or "").strip():
            self._myself_text_generator = MyselfOpenAITextGenerator.from_config()
            logger.info(
                "MYSELF_OPENAI_API configured; realtime backend will be used for ASR/TTS while text comes from %s",
                self._myself_text_generator.base_url,
            )
    ...
    def _uses_myself_text_generation(self) -> bool:
        """Return whether this handler should use the configured local text generator."""
        return self._myself_text_generator is not None
    ...
    def _schedule_myself_text_turn(self, transcript: str) -> None:
        """Schedule local text generation without blocking the realtime receiver."""
        task = asyncio.create_task(
            self._handle_myself_text_turn(transcript),
            name="myself-openai-text-turn",
        )
        self._myself_text_tasks.add(task)
        task.add_done_callback(self._myself_text_tasks.discard)

    async def _handle_myself_text_turn(self, transcript: str) -> None:
        """Generate assistant text with MYSELF_OPENAI_API and ask realtime to speak it."""
        if self._myself_text_generator is None:
            return

        try:
            reply = await self._myself_text_generator.generate_reply(
            ...
            logger.exception("MYSELF_OPENAI_API text generation failed")
            await self.output_queue.put(
                AdditionalOutputs(
                    {
                        "role": "assistant",
                        "content": f"[local text error] {type(e).__name__}: {e}",
                    }
                )
            )
            return
        ...
        self._mark_activity("myself_text_reply")
        await self.output_queue.put(AdditionalOutputs({"role": "assistant", "content": reply}))
        await self._speak_myself_text(reply)

    async def _speak_myself_text(self, text: str) -> None:
        """Use the realtime backend as TTS for text generated by MYSELF_OPENAI_API."""
        if not self.connection:
            logger.debug("Skipping MYSELF_OPENAI_API speech output because realtime connection is closed")
            return

        self._suppress_next_realtime_transcript = True
        tts_prompt = (
            "Read the following text exactly as written. Do not add, remove, summarize, translate, "
            "or comment on it.\n\n"
            f"{text}"
        )
                ...
                input=[
                    {
                        "type": "message",
                        "role": "user",
                        "content": [{"type": "input_text", "text": tts_prompt}],
                    }
                ],
                instructions="You are only rendering the provided text as speech. Say it exactly.",
            ...
            try:
                ...
                async for event in self.connection:
                    ...
                    if event.type == "conversation.item.input_audio_transcription.completed":
                        ...
                        if self._uses_myself_text_generation():
                            self._schedule_myself_text_turn(transcript)

                    # Handle assistant transcription
                    if event.type == "response.output_audio_transcript.done":
                        self._mark_activity("assistant_transcript_done")
                        logger.debug(f"Assistant transcript: {event.transcript}")
                        if self._suppress_next_realtime_transcript:
                            self._suppress_next_realtime_transcript = False
                            logger.debug("Suppressed realtime transcript for MYSELF_OPENAI_API speech output")
                        else:
                            await self.output_queue.put(
                ...
                for task in list(self._myself_text_tasks):
                    task.cancel()
                for task in list(self._myself_text_tasks):
                    try:
                        await task
